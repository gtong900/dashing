...
let API_URL = "http://Dashing-env.rsvmpxcxt5.us-west-2.elasticbeanstalk.com:80/api/admin/";
...
export class AdminService {
  ...
  numberOfUsers(): Observable<any> {
    return this.http.get(API_URL + "user-count",
      { headers: this.headers });
  }
  ...
  updateProduct(product: Product): Observable<any> {
    return this.http.post(API_URL + "product-update", JSON.stringify(product),
      { headers: this.headers });
  }
  ...
  numberOfProducts(): Observable<any> {
    return this.http.get(API_URL + "product-count",
      { headers: this.headers });
  }
  ...
  numberOfTransactions(): Observable<any> {
    return this.http.get(API_URL + "transaction-count",
      { headers: this.headers });
  }
}
